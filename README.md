...
A Python implementation of HMSmap is now available in the `pyEDM_scripts` directory. This implementation aims to be compatible with the [pyEDM](https://github.com/SugiharaLab/pyEDM) library, following its coding style and class structure. The Python implementation includes:

- `HMSmap.py`: The main HMSmap class that inherits from the EDM base class
- `HMSmap_example.py`: A simple example of how to use the HMSmap class with a logistic map
- `compare_HMSmap_SMap.py`: A script that compares the performance of HMSmap vs. standard SMap with increasing observation noise
...
- pyEDM (optional, for running comparisons with standard EDM methods)

#### Example Usage

```python
# Import the HMSmap class
from pyEDM_scripts.HMSmap import HMSmap

# Create and run HMSmap
hms = HMSmap(
    dataFrame=data,          # pandas DataFrame with time and observation columns
    ...
    vobs=0.01,               # Observation noise variance
...
# Get results
results = hms.Projection     # DataFrame with Time, Observations, Predictions, Filtered_States
coefficients = hms.Coefficients  # DataFrame with regression coefficients
```

Run the example scripts to see HMSmap in action:
```
python pyEDM_scripts/HMSmap_example.py
python pyEDM_scripts/compare_HMSmap_SMap.py
```

The comparison script will show how HMSmap outperforms standard SMap as observation noise increases.
